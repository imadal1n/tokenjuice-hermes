...
def _build_group(
    ...
    *,
    ...
    has_disposable = bool(disposable_members) and any(
        _is_disposable_member(c) for c in disposable_members
    )
    soft_allowed = has_disposable and all(
        c.prune_policy == SOFT_TRIM_POLICY for c in disposable_members
    )
    hard_allowed = has_disposable and all(_is_disposable_member(c) for c in disposable_members)
...
def _is_disposable_member(contribution: ContributionInternal) -> bool:
    return contribution.class_ in RESCUE_BACKED_CLASSES or contribution.prune_policy in {
        SOFT_TRIM_POLICY,
        HARD_CLEAR_POLICY,
    }
